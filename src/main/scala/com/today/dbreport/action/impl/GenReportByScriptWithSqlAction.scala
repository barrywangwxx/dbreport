...
/**
  ...
  * @version 0.0.1
  */
class GenReportByScriptWithSqlAction(genReportParam: GenReportParam) extends ReportAction {
  val logger= LoggerFactory.getLogger(classOf[GenReportByScriptWithSqlAction])
  /**
    * 前置条件检查：动作、状态等业务逻辑
    */
  ...
  override def genetateReport: Unit = {
    ...
    val args = Seq( ("sql", Some(genReportParam.querySql)),
                    ("driver", Some(driver)),
                    ("url", Some(url)),
