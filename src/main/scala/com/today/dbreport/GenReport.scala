...
object GenReport {
  def main(args: Array[String]): Unit = {
    val database = "member"
    val tempalte = GetDataFromDBTest.getClass.getClassLoader.getResource("templateForTest/my_template.xls").getPath.substring(1)
    ...
    val args = Seq( ("sql", Some(sql)),
                    ...
                    ("output", Some(output)))
    val script = GetDataFromDBTest.getClass.getClassLoader.getResource("scripts/genreport.sc").getPath.substring(1)
